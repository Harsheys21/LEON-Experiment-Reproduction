...
class QueryFeaturizer(Featurizer):
    ...
    def __call__(self, node):
        ...
        # print("node:", node)
        # print(f"rel_ids: {self.workload_info.rel_ids}")
        # print(f"Converted aliases: {[self.table_id_to_alias(table) for table in self.workload_info.rel_ids]}")
        ...
        # graph_nodes = list(query_join_graph.nodes)
        # missing_nodes = set(all_aliases) - set(graph_nodes)
        # print(f"Missing nodes in graph: {missing_nodes}")
        missing_nodes = set(all_aliases) - set(query_join_graph.nodes)
        query_join_graph.add_nodes_from(missing_nodes)
